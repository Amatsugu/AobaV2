...
use crate::{
	...
	rpc::{
		aoba::{
			self, FileInfo, IdList, UploadRequest, UploadResult, UploadTarget,
...
pub fn UploadArea(props: UploadAreaProps) -> Element {
	...
	let _upload_progress = use_signal(|| 0.0_f32);
	...
	let on_files_dropped = move |e: Event<DragData>| {
		...
		let _total_file_size: u64 = e.files().iter().map(|f| f.size()).sum();
...
async fn upload_files(
	files: Vec<FileData>,
	_upload_state: Signal<UploadState>,
) -> Result<UploadResult, Vec<String>> {
			...
			upload_target_response::Result::Targets(upload_targets) => {
				...
				return if !uploaded_ids.is_empty() {
						...
						.await
					{
						...
						Err(status) => Err(vec![
							format!("Failed to complete upload: {}", status).to_string(),
						]),
					}
				} else {
