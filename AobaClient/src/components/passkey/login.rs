...
) -> Result<PasskeyLoginRequest, anyhow::Error> {
	let Some(credentials) = window().map(|w| w.navigator().credentials()) else {
...
fn get_credential_request_options(
	assert: PasskeyAssertionOptions,
) -> Result<CredentialRequestOptions, anyhow::Error> {
	let opts = CredentialRequestOptions::new();

	info!("Challenge: {:?}", assert.challenge);
	let pub_key = PublicKeyCredentialRequestOptions::new_with_u8_array(&bytes_to_uint8array(
		&assert.challenge,
	));
